...
- **费用统计** — 实时追踪 API 调用次数、Token 用量和预估费用，底部状态栏即时更新
- **TTS 语音播放** — AI 回复自动朗读，支持手动播放/暂停
...
| 语音识别 | DashScope Recognition API（主）→ FunASR 本地离线（备用）→ Google Speech（降级） |
...
│   │   ├── stt_service.py       # 语音识别：DashScope Recognition + FunASR + Google 三级降级
...
FunASR 是阿里开源的本地语音识别模型，无需 API Key，可在无网络环境下使用。默认已包含在依赖中，首次使用会自动下载模型（约 1GB）。

如需跳过 FunASR 以加快安装速度，可编辑 `requirements.txt` 注释掉 `funasr`、`modelscope`、`torch` 三行。
...
| 语音播放 | AI 回复自动朗读；点击消息气泡中的喇叭图标手动播放/暂停 |
...
│  stt_service.py — 语音转文字（三级降级）           │
│                  DashScope → FunASR → Google      │
└──────────────┬──────────────────────────────────┘
               │ OpenAI 兼容 API
┌──────────────▼──────────────────────────────────┐
│            5 个 AI Provider                      │
